...
export class ControlPanelPage extends HTMLElement {
  ...
  render() {
    ...
    // 渲染左側子選單
    const menuTabs = [
      { id: 'hosts', label: t('cp.menu.hosts'), icon: `<rect x="2" y="2" width="20" height="8" rx="2" ry="2"/><rect x="2" y="14" width="20" height="8" rx="2" ry="2"/><line x1="6" y1="6" x2="6.01" y2="6"/><line x1="6" y1="18" x2="6.01" y2="18"/>` },
      { id: 'control-panel', label: t('cp.menu.controlPanel'), icon: `<rect x="3" y="3" width="7" height="9"/><rect x="14" y="3" width="7" height="5"/><rect x="14" y="12" width="7" height="9"/><rect x="3" y="16" width="7" height="5"/>` },
      { id: 'integrations', label: t('cp.menu.integrations'), icon: `<polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/>` },
      { id: 'kubernetes', label: t('cp.menu.kubernetes'), icon: `<path d="M12 2.5 20 7v10l-8 4.5L4 17V7l8-4.5z"/><circle cx="12" cy="12" r="2.5"/><path d="M12 5.5v4M12 14.5v4M6.5 9l3.5 2M14 13l3.5 2M17.5 9 14 11M10 13l-3.5 2"/>` },
      { id: 'keychain', label: t('cp.menu.keychain'), icon: `<rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>` },
      { id: 'forwarding', label: t('cp.menu.forwarding'), icon: `<polyline points="17 11 21 7 17 3"/><path d="M3 17h8a4 4 0 0 0 4-4V7"/>` },
      { id: 'snippets', label: t('cp.menu.snippets'), icon: `<polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/>` },
      { id: 'known_hosts', label: t('cp.menu.knownHosts'), icon: `<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>` },
      { id: 'logs', label: t('cp.menu.logs'), icon: `<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>` }
    ];
          ...
          <div class="vault-scroll-content" style="flex: 1; min-height: 0; overflow-y: auto; display: flex; flex-direction: column; gap: 24px;">
            ...
            <div class="vault-section">
              ...
              <div id="infoComponentsGrid" class="vault-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 14px; margin-bottom: 10px;">
                ${infoComponents.map(c => renderCard(c)).join('') || `<div style="color: var(--color-text-muted); font-size: 12.5px; padding: 10px 0; text-align: left;">${t('cp.empty.info')}</div>`}
              </div>
            </div>
            ...
            <div class="vault-section">
              ...
              <div id="switchComponentsGrid" class="vault-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 14px; margin-bottom: 10px;">
                ${switchComponents.map(c => renderCard(c)).join('') || `<div style="color: var(--color-text-muted); font-size: 12.5px; padding: 10px 0; text-align: left;">${t('cp.empty.switch')}</div>`}
              </div>
            </div>
            ...
            <div class="vault-section">
              ...
              <div id="functionComponentsGrid" class="vault-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 14px; margin-bottom: 10px;">
                ${functionComponents.map(c => renderCard(c)).join('') || `<div style="color: var(--color-text-muted); font-size: 12.5px; padding: 10px 0; text-align: left;">${t('cp.empty.function')}</div>`}
              </div>
            </div>
          </div>
        </div>
        ...
        <div id="componentDrawer" class="vault-drawer ${state.drawerOpen ? 'open' : ''}" style="width: 380px; background: #0c121f; border-left: 1px solid rgba(23,107,135,0.25); display: flex; flex-direction: column; transition: transform 0.3s ease; transform: ${state.drawerOpen ? 'translateX(0)' : 'translateX(100%)'}; position: relative;">
          ${!state.selectedComponent && !state.drawerOpen ? `
            <div style="flex: 1; display: flex; align-items: center; justify-content: center; padding: 40px; color: var(--color-text-muted); text-align: center;">
              <div>
                <h3 style="margin-bottom: 8px; color: var(--color-text);">${t('cp.intro.title')}</h3>
                <p style="font-size: 12.5px;">${t('cp.intro.desc')}</p>
              </div>
            </div>
          ` : `
                  ...
                  <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 16px;">
                    <label style="display: flex; flex-direction: column; text-align: left; gap: 6px; font-size: 12px; color: var(--color-subtext);">
                      ...
                      <select class="no-drag" id="compColorSelect" style="background: #0d121f; border: 1px solid rgba(23,107,135,0.2); padding: 8px 12px; border-radius: 6px; color: var(--color-text);">
                        <option value="#176b87" ${drawerComp.color === '#176b87' ? 'selected' : ''}>${t('cp.color.classicBlue')}</option>
                        <option value="#FF9900" ${drawerComp.color === '#FF9900' ? 'selected' : ''}>${t('cp.color.awsYellow')}</option>
                        <option value="#E95420" ${drawerComp.color === '#E95420' ? 'selected' : ''}>${t('cp.color.ubuntuOrange')}</option>
                        <option value="#2ecc71" ${drawerComp.color === '#2ecc71' ? 'selected' : ''}>${t('cp.color.ecoGreen')}</option>
                        <option value="#ef4444" ${drawerComp.color === '#ef4444' ? 'selected' : ''}>${t('cp.color.warningRed')}</option>
                        <option value="#a855f7" ${drawerComp.color === '#a855f7' ? 'selected' : ''}>${t('cp.color.nebulaPurple')}</option>
                        <option value="#ec4899" ${drawerComp.color === '#ec4899' ? 'selected' : ''}>${t('cp.color.rosePink')}</option>
                        <option value="#06b6d4" ${drawerComp.color === '#06b6d4' ? 'selected' : ''}>${t('cp.color.glacierCyan')}</option>
                        <option value="#f59e0b" ${drawerComp.color === '#f59e0b' ? 'selected' : ''}>${t('cp.color.amberGold')}</option>
                        <option value="#10b981" ${drawerComp.color === '#10b981' ? 'selected' : ''}>${t('cp.color.mintEmerald')}</option>
                        <option value="#6366f1" ${drawerComp.color === '#6366f1' ? 'selected' : ''}>${t('cp.color.indigoNight')}</option>
                        <option value="#f43f5e" ${drawerComp.color === '#f43f5e' ? 'selected' : ''}>${t('cp.color.coralCrimson')}</option>
                        <option value="#14b8a6" ${drawerComp.color === '#14b8a6' ? 'selected' : ''}>${t('cp.color.tealHorizon')}</option>
                        <option value="#64748b" ${drawerComp.color === '#64748b' ? 'selected' : ''}>${t('cp.color.spaceSlate')}</option>
                      </select>
                    </label>
                    <label style="display: flex; flex-direction: column; text-align: left; gap: 6px; font-size: 12px; color: var(--color-subtext);">
                      ${t('cp.field.type')}
                      <select class="no-drag" id="compTypeSelect" style="background: #0d121f; border: 1px solid rgba(23,107,135,0.2); padding: 8px 12px; border-radius: 6px; color: var(--color-text);">
                        <option value="function" ${drawerComp.type === 'function' ? 'selected' : ''}>${t('cp.type.function')}</option>
                        <option value="info" ${drawerComp.type === 'info' ? 'selected' : ''}>${t('cp.type.info')}</option>
                        <option value="switch" ${drawerComp.type === 'switch' ? 'selected' : ''}>${t('cp.type.switch')}</option>
                      </select>
                    </label>
                  </div>
  ...
  setupListeners() {
    ...
    if (form) {
      form.addEventListener('submit', (e) => {
        ...
        if (type === 'info') {
          ...
          if (items.length === 0) {
            showToast(t('cp.toast.infoNeedItem'), { type: 'error' });
            return;
          }
          ...
          if (!queryCommand) {
            showToast(t('cp.toast.needQuery'), { type: 'error' });
            return;
          }
          if (!stateALabel || !stateBLabel) {
            showToast(t('cp.toast.needStateLabels'), { type: 'error' });
            return;
          }
          ...
          if (!remoteCommand && !localCommand) {
            showToast(t('cp.toast.needCommand'), { type: 'error' });
            return;
          }
    ...
    const performExport = async (format) => {
      try {
        ...
        if (res && res.success) {
          showToast(t('cp.toast.exportOk'), { type: 'success' });
        } else {
          showToast(t('cp.toast.exportFail', { error: res ? res.error : t('cp.unknownError') }), { type: 'error' });
        }
      } catch (err) {
        showToast(t('cp.toast.exportFail', { error: err.message }), { type: 'error' });
      }
    };
    ...
    const performImport = async (format) => {
      try {
        ...
        if (!res || !res.success) {
          showToast(t('cp.toast.readFail', { error: res ? res.error : t('cp.unknownError') }), { type: 'error' });
          return;
        }

        const data = JSON.parse(res.output);
        if (!data || !data.components) {
          showToast(t('cp.toast.invalidFormat'), { type: 'error' });
          return;
        }

        if (!(await confirmDialog(t('cp.confirm.importMsg'), { title: t('cp.confirm.importTitle') }))) {
          return;
        }
        ...
        showToast(t('cp.toast.importOk'), { type: 'success' });
        controlPanelStore.getState().loadComponents();
        this.render();
        this.setupListeners();
      } catch (err) {
        showToast(t('cp.toast.importFail', { error: err.message }), { type: 'error' });
      }
    };
  ...
  async executeComponent(comp, card = null) {
    if (comp.type === 'switch') {
      showToast(t('cp.toast.switchInfo'), { type: 'info' });
      return;
    }
    ...
    if (!activePaneSessionKey || !tState.sessions[activePaneSessionKey]) {
      showToast(t('cp.toast.noSession'), { type: 'error' });
      return;
    }

    const session = tState.sessions[activePaneSessionKey];
    if (session.isLocal || session.isLogView) {
      showToast(session.isLogView ? t('cp.toast.notSupportedLog') : t('cp.toast.notSupportedLocal'), { type: 'error' });
      return;
    }

    if (comp.type === 'info') {
      showToast(t('cp.toast.infoAutoPoll'), { type: 'info' });
      return;
    }
    ...
    const cmdLines = [];
    if (remoteCmd) cmdLines.push(`${t('cp.exec.remoteTag')} ${remoteCmd}`);
    if (localCmd) cmdLines.push(`${t('cp.exec.localTag')} ${localCmd}`);
    if (cmdLines.length > 0) {
      if (!(await confirmDialog(t('cp.confirm.execMsg', { commands: cmdLines.join('\n') }), { title: t('cp.confirm.execTitle'), danger: true }))) {
        return;
      }
    }
    ...
    try {
      ...
      if (!result.success) {
        const message = result.phase === 'remote'
          ? t('cp.exec.remoteFail', { error: result.error || t('cp.unknownError') })
          : result.phase === 'export'
            ? t('cp.exec.exportFail', { error: result.error || t('cp.unknownError') })
            : t('cp.exec.localFail', { error: result.error || t('cp.unknownError') });
        showToast(message, { type: 'error', title: t('cp.exec.failTitle') });
        return;
      }
      // 過長輸出不塞進 toast，僅顯示完成摘要（輸出已寫入既有日誌/終端）。
      showToast(t('cp.exec.done', { name: comp.name }), { type: 'success' });
    } catch (err) {
      showToast(t('cp.exec.error', { error: String(err) }), { type: 'error', title: t('cp.exec.failTitle') });
    } finally {
      teardownBusy();
    }
  }
  ...
  setComponentCardBusy(card, busy) {
    ...
    if (busy) {
      ...
      if (!card.querySelector('.component-card-spinner')) {
        ...
        spinner.setAttribute('aria-label', t('cp.exec.running'));
